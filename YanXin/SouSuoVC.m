...
#import "FirstModel.h"
@interface SouSuoVC ()<UITableViewDelegate,UITableViewDataSource,UISearchDisplayDelegate>
...
-(void)jiexishuju
{
    [ShuJuDataModel huoquFirstPage:@"0" Tiao:@"0" success:^(NSDictionary *dic) {
        NSMutableArray * arrContent =[dic objectForKey:@"content"];
        for (NSDictionary * dicc in arrContent) {
            NSString * title = [dicc objectForKey:@"title"];
            [_dataArray addObject:title];
            model=[[FirstModel alloc]initWithDic:dicc];
