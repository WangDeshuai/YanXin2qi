...
@synthesize autoWhiteRatio = _autoWhiteRatio;
...
- (AutoHeight)autoWhiteRatio
{
    __weak typeof(self) weakSelf = self;
    
    if (!_autoWhiteRatio) {
        _autoWhiteRatio = ^(CGFloat ratioaValue) {
            //weakSelf.needsAutoResizeView.autoHeightRatioValue
          //  = @(ratioaValue);
           // weakSelf.needsAutoResizeView.width = value;
            weakSelf.needsAutoResizeView.sd_maxWidth=@(ratioaValue);
           // weakSelf.needsAutoResizeView.fixedWith = @(ratioaValue);
            return weakSelf;
        };
    }
    return _autoWhiteRatio;
}
