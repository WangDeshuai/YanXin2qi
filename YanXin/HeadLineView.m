    ...
    if (buttonArray.count>0) {//防止没创建前为空
        for (UIButton *labelView in buttonArray) {
            if (labelView.tag==index) {
                if (labelView.tag==0) {
                    //深绿线，选中之后的颜色
                    label1.backgroundColor=DAO_COLOR;//JXColor(179, 211, 85, 1);
                }else if(labelView.tag==1){
                    label2.backgroundColor=DAO_COLOR;//JXColor(179, 211, 85, 1);
                }else{
                    label3.backgroundColor=DAO_COLOR;//JXColor(179, 211, 85, 1);
                }
